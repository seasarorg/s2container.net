...
namespace Quill.Ado.Tests {
    ...
    public class SqlPropTests {
        ...
        public void CreateByPrametarizedSQL2_Test() {
            // Arrange
            const string sql = "SELECT * FROM Hoge LEFT OUTER JOIN Huga ON Hoge.ID = Huga.ID AND Huga.Name = /* hname */'aiueo' WHERE ID = /* id */10";
            const string expectedSql = "SELECT * FROM Hoge LEFT OUTER JOIN Huga ON Hoge.ID = Huga.ID AND Huga.Name = @hname WHERE ID = @id";
